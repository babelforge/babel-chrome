...
BabelChrome is a macOS browser workspace built around a native CEF-based application and an installable module system. The current implemented module runtimes are `php-web`, `php-class`, `static-web`, `process-web`, and `process-runtime`, and the module contract is runtime-aware rather than PHP-only.
...
Modules are independent extension projects shipped as zip packages. The implemented runtime handlers support PHP modules such as Symfony apps, Laravel apps, plain PHP front controllers, PHP class entrypoints, static web modules that need no PHP entrypoint, local HTTP process modules, and non-web process modules. The manifest contract keeps these runtime families behind the same stable module surface.
