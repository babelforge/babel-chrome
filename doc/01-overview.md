...
The browser provides the native shell, tab model, groups, address bar, native module registry and installer, native process-web runtime actions, local service host, stable `babelchrome://` routes, and runtime integration with macOS. Modules provide optional capabilities such as Markdown rendering, OpenAPI rendering, JSON rendering, local project launching, and framework integration examples.
