...
- an optional local update folder, used as fallback when no URL is configured or when the URL cannot be read.

The release manifest is the file generated by `tools/prepare-module-release.sh`. When a newer module version is found, the update report offers an `Install Update` action that installs the matching zip from the active source.
